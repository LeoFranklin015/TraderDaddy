...
function calculateSentiment(votes: any) {
  const totalVotes =
    votes.positive +
    votes.negative +
    votes.important +
...
interface Post {
  text: string;
  sentiment: ReturnType<typeof calculateSentiment>;
}

export async function processSentimentCryptoPanic(asset: any, agent: any) {
  const posts: Post[] = [];
